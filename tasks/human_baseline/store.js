...
function serve() {
  ...
  mongoConnectWithRetry(2000, (connection) => {
    ...
    app.post('/db/getstims', (request, response) => {
      ...
      collection.distinct("numGames", (error, values) => {
	const query = {'numGames' : {$eq: _.min(values)}};
	collection.find(query, {}, (err, cursor) => cursor.toArray((err, all_games) => {
          if(err) {
            console.log(err);
          } else {
	    const results = _.sample(all_games);
