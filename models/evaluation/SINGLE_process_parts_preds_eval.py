...
########CHANGE##########
image_path='../data/heldout/images/color' ##CHANGE
image_files = [f.replace('.png','') for f in listdir(image_path) if isfile(join(image_path, f) and f.endswith('.png'))]

loadpaths=['./vilt/whole_color/heldout_w+c_OOB.pth'] ##CHANGE
dict_keys=['OOB'] ##CHANGE

save_path = './vilt/accuracies/heldout_OOB_w+c.json' ##CHANGE
...
for imgfile in image_files:
  ...
  file_corrects = np.zeros((len(dict_keys),2)) # one model, each element (text pred, image pred)
